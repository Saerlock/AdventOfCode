...
function getNumberFormated(text) {
    ...
    return textTab.map((elem) => {
                const match = elem.match(/([\d]|one|two|three|four|five|six|seven|eight|nine)/g);

                let first = match[0];
                let last = match[match.length - 1];            

                if (!first.match(/[0-9]/)) {
                    const matchFiltered = elem.replace(first, first.charAt(0) + first + first.charAt(first.length-1)).match(/[\d]|one|two|three|four|five|six|seven|eight|nine/g);
                    last = matchFiltered[matchFiltered.length - 1];     
...
console.log(getNumberFormated('1sevenseven71d\n'));
